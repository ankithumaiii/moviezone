...
export class OpenCinemaProvider implements IWebProvider {
  ...
  public supportedTypes = ['open_movie', 'movie', 'short'];
  ...
  public async loadDetails(id: string): Promise<MediaItem | null> {
    const item = OPEN_CINEMA_ITEMS.find((m) => m.id === id);
    return item || null;
  }

  public async loadSources(id: string): Promise<NormalizedSource[]> {
    const item = await this.loadDetails(id);
  ...
  public canDownload(source: NormalizedSource): boolean {
    return Boolean(source.downloadable && source.downloadUrl);
  }

  public async download(source: NormalizedSource): Promise<string | null> {
    if (this.canDownload(source) && source.downloadUrl) {
